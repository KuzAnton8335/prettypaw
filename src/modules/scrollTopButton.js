...
const creatArrow = (className = 'arrow-up', { hover = true } = {}) => {
	...
	style.textContent = `
	  .${className} {
	   ...
	   right:30px;
		...
		display:none;
		${hover && 'transition:color .3s easy-in-out, background-color .3s easy-in-out;'};
	  }
...
export const initScrollTopButton = (className, hover) => {
	const arrow = creatArrow(className, hover);
	...
	let n2 = 0;
	const showElemScrollPosition2 = () => {
		console.log('scroll2', n2++)
		const scrollPosition = window.scrollY || document.documentElement.scrollTop;
	...
	window.addEventListener('scroll', debounce(showElemScrollPosition2, 100));
}